...
#### 파급력 및 기대효과
 ...
 3) 공정 실시간 모니터링
 4) 설비 유지 보수 용이  
 소비자의 개성이 뚜렷해지면서 다품종 소량생산의 필요성이 증가하고 있다. 다품종 소량생산의 경우 생산성이 떨어질 수 있으며 공정 라인의 설계가 복잡해질 수 있다. 우리 제품은 불량 검출과 제품 분류를 한 번에 할 수 있어 공정 라인의 설계를 더 단순히 할 수 있다. 기존의 공정들은 공정의 순서에 맞게 긴 라인으로 형성되어 있고, 한 번 형성된 라인은 쉽게 바꿀 수 없다. 자율 주행 로봇은 이를 해소함과 동시에 무거운 제품을 작업자들이 이동하며 발생할 산업 재해를 예방하기도 한다. 하지만 규격화된 자율 주행 로봇은 제품의 규격에 비해 로봇의 사양이 배 이상 크다면 비효율적일 수 있다. 이 점을 착안해 드론의 군집 비행을 모티브로 협력 운송이 가능하도록 설계하였다. 협력 운송으로 인해 각각 다른 제품의 규격에 적합한 운송이 가능하며, 이는 서버를 통해 관리하므로 효율적인 이용이 가능하다. 마지막으로, 장비가 모듈화되어 설비의 유지 보수가 간편하다.
***
## 팀명
